...
def main():
    ...
    message = "ABC QRSTU 2017 = 46 = оEÏHΑ ]υᛖЖΠ ιPOOZ 8#Ёép ᚦLτ£K СeÖμ§ ÅΤΟθø Ζ.ᛋοᛯ Ю#у_Я ᚬеjγα ГwνB, µ"
    # encipher = True
    encipher = False
    date = None
    month = None
...
class Enigma_M3():
    ...
    # 'XXX': ['XXX', 'ρþйЫᚦU#ёᚺ0пДКэ<;ΟÖQᚨpáΙᛃвъï7nИСц]([ПᛯᛏᚾMNΝ5+Fωexο,ьκdLгΧΗuаᚷjYЕᚲΓvμᛒυΩФΕøsJΠπЦЮqз)ΘᚬΦшÆ^εξШх4τᛞ-ᚴУГᛦÏБyΑΥ3ЪAНζð$Оᛁg.æΒíÓÁлw`9σ\'XψдoфжюΖХТÐЭᚼᛟᛖеᛅᛇiыfуTrᚠЯ{нaЧчóηᚱᛰ@ᛈSМ\\ᚢяктZиtрВCkPΡÄЖγαKщιδ}ΣᛘRР%zΞ>ᛮIᛚl!ö*ЛΔØ8ΚΤбᛉΛᛋΨχmcEHν/ᛗφDЙbΜG&м|ЩéᚹβЗÍсO1å~Ё6:Þ2Аh=ÅäÉ?ᛜ"ᛊW_оλθЬVB', ['!', '#', '$', '%', '&', '(', '-', '.', '1', '2', '5', '<', '?', 'H', 'I', 'L', 'P', 'R', 'X', '\\', 'b', 'e', 'g', 'k', 'n', 'p', 'q', 'r', 's', 'v', '|', '}', '~', 'É', 'Þ', 'Æ', 'é', 'ð', 'ó', 'þ', 'Ε', 'Κ', 'Ξ', 'Ο', 'Χ', 'Ψ', 'ε', 'ζ', 'μ', 'υ', 'Б', 'В', 'Д', 'Е', 'Ж', 'Н', 'С', 'Ш', 'Э', 'б', 'д', 'ё', 'ж', 'н', 'р', 'т', 'у', 'ц', 'ч', 'ш', 'ы', 'ᚴ', 'ᛅ', 'ᛇ', 'ᛈ', 'ᛘ', 'ᛦ', 'ᛰ']],
    # 'XXXI': ['XXXI', 'ДСБηΩ0въКιDФыζ@Ю/-Шфᛦgᛘ|åΖéᚲÓ";ΠИншw_kВᚢAκaÉЦÁ5йoЖᚱояᚠиХЗEᛟÞᚬΑb[4f}Γ~зЪ,ΛοeuÄМ%inM.ᛏlN32=YKьγðчᛃØLО{ᚾщпψ:7υ6PG>ΜᚨΗдóУᛉᛅТэ*UЕᛁΤλ&ÆΞνWpδÐÖ(ΚzцᚺΣжᛋCïä#р)ёЧτÅHмЬJπσаΟZÏjПᛊЙyлᚴΙсΨμsvᛯ]РᛚαЛНrбtxхᛜξГ?RᛰθЫᚷö`dᚼþᚹ<ᛒSΝβ1ᛗ^уáΒcqh$Ε8!еАΘΡЭæᛈ+ΔIφOεVкЩᛖΧю\'ÍχTᛮQЯω\\ρᛇт9øгXBFΦЁᚦímᛞΥ', ['*', '/', '4', '9', ':', '@', 'O', 'P', 'U', 'X', '[', '_', 'b', 'd', 'f', 'i', 'n', 'r', 't', 'z', '|', '}', 'Ð', 'Ó', 'Æ', 'ï', 'ó', 'þ', 'Β', 'Ζ', 'Ξ', 'Σ', 'Υ', 'Φ', 'Χ', 'Ω', 'ε', 'ι', 'κ', 'π', 'ρ', 'σ', 'τ', 'υ', 'χ', 'А', 'И', 'Л', 'П', 'Р', 'Т', 'У', 'Ф', 'Я', 'и', 'й', 'м', 'ш', 'ъ', 'ь', 'э', 'ю', 'я', 'ᚦ', 'ᚬ', 'ᚱ', 'ᚺ', 'ᚾ', 'ᛃ', 'ᛅ', 'ᛇ', 'ᛋ', 'ᛏ', 'ᛗ', 'ᛜ', 'ᛞ', 'ᛯ', 'ᛰ']],
    # 'XXXII': ['XXXII', ';pМÓ[oDо/!Mл}`ï2NᛈΠᚨ&χ,щяФqъενξxOl(ζᚲΒÆÁ1gCΨΦ")%эXᚾсЗFΑᛞᚺβηᚷ?uᚬρ8Δυ0YvδγeψпᛰΝPkᛖzдAUёyж>Оá*Ι$bКᛯᚴЁй@ᛏ=ᛊЩ<4ЕΤóᚹðΕ]ΡЧåРЫΚЬBτÄьСᛦnλБiaμΖΘΞjöᛉ{ЖеøΩУᛋαШучÉЮwюΥ6Т+ГᚢᛒιéEмΣ_sЯДΧᚠmοΛL3НI\\ᛃᛟПÍωÖ^гdQаTκᛘ.Хᚼr~-φþcπшVз\'АЪÏхJΟKкᛅᛚSäᛁВ:ᛇ#ᚱΗрфЭЙhσ7ÞfЦИWΓθᛮRæZвциЛØÐtнH|ÅΜ9ᛗᚦыбт5ᛜGí', ['#', '$', '%', '&', '*', '+', '/', '1', '4', '6', '9', ':', ';', '=', '@', 'H', 'J', 'K', 'L', 'M', 'P', 'R', 'V', 'Y', 'Z', '[', ']', 'a', 'e', 'h', 'i', 'n', 'p', 'q', 'r', 'u', 'v', 'z', 'Ä', 'é', 'ï', 'þ', 'æ', 'å', 'Ζ', 'Κ', 'Ο', 'Ρ', 'Σ', 'Τ', 'Υ', 'Ω', 'δ', 'ζ', 'θ', 'μ', 'ο', 'φ', 'Б', 'В', 'Г', 'Й', 'О', 'Т', 'Ч', 'Ь', 'Я', 'а', 'в', 'д', 'е', 'ё', 'м', 'н', 'х', 'ц', 'ы', 'ю', 'ᚦ', 'ᚴ', 'ᚷ', 'ᚹ', 'ᚾ', 'ᛁ', 'ᛅ', 'ᛈ', 'ᛊ', 'ᛖ', 'ᛰ']],
    # 'XXXIII': ['XXXIII', 'ᚨ7СkxTÉн*ηVЖgo~ΦÅBKаБ1PτajIЮRЛαЦЗЁᚺΨвÐᛚт}ᛈΓЫΡΛзπΝ-<Eενй?Y|SμХyЭᚲ!8ᛁJо.ωQlρᛯHΖNЕᚠгΙÖΔ`ᛰ3:ΑВ"bAðÞíᚹл5ШᛏᛅᛞРζДUeжþοФ0дΒИpåÍᚾхэCТΥ]ᛋξ;uøΗóяᚴ+кᛗhMсΘш9пΚnÓᛒWγFЙÏáZбOЩᛉz=ЪΩvæXᚼᛊУ@ΟrᛜᚬÄе\'δ&ъюΣ>ᛟΜ(ᛘΞ6G/фиᛃёäрᛦÁΧщéмöβᛮιq4Чυ,Кᚢ_sï^Ø\\ΤφᚦκцmᛖθfψЯ$cч{ᚷьΠ2ᚱλ%ᛇ)σ#МLП[tНАΕidыуÆЬχОDwГ', ['"', '$', "'", '*', ',', '/', '2', '3', '4', '8', ';', 'D', 'J', 'L', 'N', 'O', 'R', '^', 'e', 'i', 'k', 'p', 'q', 's', 't', 'w', 'y', 'z', '}', 'Á', 'É', 'Ð', 'Ó', 'Þ', 'Å', 'ð', 'þ', 'æ', 'ä', 'Β', 'Ε', 'Μ', 'Π', 'Σ', 'Υ', 'Χ', 'Ψ', 'Ω', 'α', 'β', 'ζ', 'η', 'ι', 'μ', 'ξ', 'π', 'χ', 'ω', 'Б', 'В', 'З', 'Л', 'О', 'Р', 'У', 'Ф', 'Ч', 'Ъ', 'Ь', 'Э', 'б', 'г', 'ж', 'й', 'л', 'м', 'о', 'п', 'т', 'я', 'ᚨ', 'ᚲ', 'ᚴ', 'ᚺ', 'ᛅ', 'ᛉ', 'ᛋ', 'ᛒ', 'ᛚ', 'ᛜ', 'ᛰ']],
    # 'XXXIV': ['XXXIV', 'Ёзуj<ᚱюηя+óБÄqЪдΑØ/áYΞЖ|чΝζиᛞ#Æрᛜᛁχᚾd]мПRI.К=υᚦÁМаPψэZJrlжЩ{6øΕЮs9ᚼmue}íΓᛊ\\ᚢᛦцæёᛒᛯQ\'ЙЭaιθÉΙтᛖ~ΘtъοkᛰöᚲТFwM_Åc*Cεβ@&ÐАþXЯνлΨЛWᛮнᛅρУ;>ЧïΩN1ᚴХz^nйγфLΗOVСg4ЕщВδ0ИμΦᚠыΟᚺᚹðФÍ-p?ᛋЬ:byv3τUåᛗоSᚬDᛉκ`ΠᛏΔОᛇπхΖT$с%пᚷBAΚxᛃЗäéΣгНoKλω5Τ,φГΧ)"!Υ[ШЫfᚨшÏΡξьЦРᛘкσÓᛈе8вαiΒбᛚ7GEΛΜÖᛟ(ÞHhД2', ['$', '.', '0', '1', '3', '4', '5', '6', ':', 'A', 'B', 'D', 'F', 'I', 'J', 'K', 'M', 'P', 'U', 'W', 'X', 'Y', 'f', 'm', 'q', 'w', 'x', 'Á', 'Ï', 'Ð', 'í', 'ó', 'Α', 'Ι', 'Μ', 'Ο', 'Σ', 'Χ', 'Ψ', 'Ω', 'ζ', 'λ', 'μ', 'ξ', 'ρ', 'σ', 'τ', 'Н', 'П', 'Р', 'Ц', 'Ч', 'Ш', 'Ъ', 'Ь', 'Я', 'а', 'ё', 'з', 'и', 'й', 'о', 'р', 'с', 'т', 'ф', 'х', 'ш', 'я', 'ᚲ', 'ᚴ', 'ᚷ', 'ᚹ', 'ᚺ', 'ᛅ', 'ᛚ', 'ᛦ', 'ᛮ']],
    # 'XL': ['XL', 'Ð@ПÏСу#!"ke0ᛟвᚨБцЮ2,ïкÄμᛋᚷ^ΓwZTаí~á1г/ØᛉГyᚢΡ%ᛊъ6ÖИЬЙfЕмΣ5oσфrёЪиΦщЦVᚬDοК{ΧрбiρpνΩaᚼю.9&vᛁ7ОIΚéöHßtоÆ|АᛞSΖmХᚠÍWυᚴ<Á;ᛈφΤсхΥæэйQζ4ᛚLЧᛦøu\'Ns$лШU-ηᛏXGj)EÉᛘᚱ?ᛯMóAωΑзqЁМΗДЩÞxΟγBαжᛇᚲЖΕᛖΨ>3ðψJ_dO:CΝптᚺΠΙåβьЛᛜKш]κ+ᛗhНыΘЭ*дУнᛮτε=FᛰᚦРΛᛅP}ᛃgcяδlÓЯθчξþеπäzιΔВb8(`λᚾYÅФЫТΒΞnᛒᚹRЗΜ[χ', ['!', '$', '*', '/', '1', '2', '3', '6', '8', ';', '<', '=', 'D', 'F', 'H', 'S', 'W', '[', '_', 'a', 'e', 'f', 'g', 'i', 'j', 'z', '|', 'Ï', 'Þ', 'Ä', 'Ø', 'Ö', 'Å', 'é', 'ð', 'æ', 'ä', 'Β', 'Δ', 'Ε', 'Η', 'Ο', 'Σ', 'Υ', 'Φ', 'α', 'β', 'γ', 'ζ', 'ι', 'ν', 'π', 'ψ', 'А', 'Д', 'Е', 'Ё', 'Ж', 'З', 'Л', 'М', 'Н', 'О', 'П', 'Ф', 'Х', 'Ь', 'Э', 'Ю', 'Я', 'а', 'б', 'ж', 'м', 'н', 'с', 'у', 'ф', 'ц', 'щ', 'ъ', 'ю', 'я', 'ᚦ', 'ᚬ', 'ᚴ', 'ᚷ', 'ᚹ', 'ᚺ', 'ᛖ', 'ᛚ', 'ᛜ']],
    # 'XLI': ['XLI', 'lξp7ᛃᛮxЁА6ᚾ]ΤИйюÞτΥυТх4Р_iжβΓTåuЫΒЭVäтᛯ+цMSλðνбÄgᛇᛏFᛁЛ;bσᛉЗᛘ~рᛚcΧOßНBdᛜ=qöψС&<ᚢ}М{ᚹ.NoлΚаaХ(ЯЮη^Qø21#ΑВ"eÁᚦÏгζᛊеfÖ%PФHÉуóиXᚨБαᛅο>кsЩΦχYщ8ΜΝΠьОÍZᚴkΔΡρШЖЕ)очι@IEЬнéÆᛈыyК?сLᚱэᛦΨᚲDΣᚷW`ёᛖЦzΩvΘшΗÐ/:ДþᚬΙwфмØRGзCᛰᛟ*9rKУ$φπUj,ᚠяᚼ-дᛞΖᛒÓí0n!Пæ[ΞЙδωΕθвъΟᛗtAáï5ЪГ|Ч\'Åγ3κmпμεᛋᚺhΛJ', ['"', '%', '*', '-', '.', '/', ':', ';', '<', '@', 'A', 'B', 'I', 'J', 'L', 'R', 'S', 'X', '[', '_', '`', 'a', 'e', 'i', 'k', 'o', 's', 't', 'u', 'x', 'y', 'z', '|', 'Á', 'Ï', 'Ð', 'Ä', 'é', 'ï', 'ð', 'ó', 'ø', 'Β', 'Γ', 'Ε', 'Ζ', 'Κ', 'Ξ', 'Ο', 'Σ', 'Υ', 'Φ', 'Ω', 'α', 'γ', 'ε', 'ζ', 'θ', 'ρ', 'σ', 'φ', 'ω', 'А', 'Б', 'Д', 'Ё', 'Й', 'О', 'С', 'Щ', 'Ь', 'а', 'д', 'е', 'ё', 'з', 'л', 'м', 'п', 'ф', 'ц', 'ъ', 'ы', 'я', 'ᚦ', 'ᚴ', 'ᚺ', 'ᛇ', 'ᛒ', 'ᛘ', 'ᛟ', 'ᛯ']],
    # 'XLII': ['XLII', '/*ΞßCιᛏKÄᚲᛘUΖΒКёы!Φ}ηéJᛜЫjФᚹн%яdζᛇц(ИьᚺЦЗᛈюεэvШ5ᛰВVТᚴΨЮøЛψOф_#Ó]ᛟφ.ÏΥy\'D8Ðκ^ᛊᚱ~ᛯП4еRπ|МωZÞÉгnwuÅΟΘeΩйᛖоνᛅ"áYᛚ9σf`ОΡLPαЕᛁ?-:ïÖυсTРЁΚGSóхЖᚼHᛦβÁοmq;gíλΔНпNᚬΑΠикÆbтåμмAΗtХЙЧуcᚾk3Τp>ᛉM[Ь=Ν7γᛃᛋξxДæАhδаIþ@ᚠᚨBщ<лΣäЪΕᛗÍбв+з{ØГrχθдsðρ&чЯЩEрΧoᚢᚷУшi,ᚦ$ᛒΛl)zτΜЭ6Fᛮ2БQъX1ΓöС0WᛞaжΙ', ['#', '$', '&', "'", ')', '*', '/', '0', '3', '4', '7', '8', '9', '?', 'G', 'H', 'J', 'U', 'Z', ']', '`', 'd', 'g', 'h', 'i', 'o', 'v', 'y', 'z', '}', 'É', 'Í', 'Ð', 'Þ', 'Ø', 'á', 'ð', 'þ', 'æ', 'å', 'ß', 'Λ', 'Π', 'Σ', 'β', 'γ', 'ε', 'ζ', 'ξ', 'ο', 'π', 'φ', 'ω', 'В', 'Г', 'И', 'Н', 'О', 'П', 'Р', 'У', 'Ц', 'Ш', 'Ь', 'Я', 'ж', 'и', 'й', 'м', 'п', 'ц', 'щ', 'ь', 'э', 'ᚠ', 'ᚬ', 'ᚴ', 'ᚹ', 'ᚺ', 'ᚼ', 'ᛇ', 'ᛋ', 'ᛏ', 'ᛖ', 'ᛘ', 'ᛞ', 'ᛟ']],
    # 'XLIII': ['XLIII', 'WΙДл_ωÁ#тъδMPöψðа>нᚱγyК[nЩ3ΜπᛖzΑШᛁυᛉГ"ΣRTαᚷÓÄиJΓᚼQᛏё$ᛜфEφΗΞUзØΔΟ1*plVá\'шТ~IЮχåяᛒλᛞÉXζЫ&ᛟq/еΛᛚᛗᛇᛈGΘYэZνíО`п?ВD]кξuiрsøцᚨ8Í,ÆΖᛯcΕ}ΧᛃоР%(0ЛМtбΒaθeäΝᚹЙЪσßХᚦА)ПgAdЧΩH9Ц5хю7ᛅÖ!сЯЭчéΤᚺLκыμ+ьjвrᚴb<ómᚢΦÞρᛮ:Иᛊ4ж.ÐᛘkᚾKЁηᛋᛦᚲйCЕΨτN|БÏSxιοΡhЬ;εΥᛰᚬÅФ=βгНΚ-6OfЖм2æwΠ@ovдуУBᚠ^þïЗщС{F', ['#', '%', '+', '.', '1', '2', '5', '<', 'A', 'D', 'H', 'L', 'N', 'Q', 'S', 'W', 'X', 'Z', 'f', 'h', 'l', 'm', 'n', '{', '|', '~', 'Í', 'Ó', 'Þ', 'Ö', 'í', 'ó', 'æ', 'å', 'Α', 'Δ', 'Η', 'Θ', 'Μ', 'Τ', 'γ', 'δ', 'ε', 'ι', 'κ', 'φ', 'χ', 'А', 'В', 'И', 'К', 'М', 'П', 'С', 'Х', 'Щ', 'Э', 'а', 'д', 'ё', 'ж', 'м', 'н', 'х', 'ч', 'ш', 'ы', 'ᚠ', 'ᚷ', 'ᚺ', 'ᚾ', 'ᛇ', 'ᛉ', 'ᛋ', 'ᛖ', 'ᛯ']],
    # 'XLIV': ['XLIV', 'MZᚱå"a0ЩΝьΦᚠыιᛚoQ+)ε?νХJ-ᛉL7хV{PΤᚼᛅ&kGÆAᚾᚦгÖ#вᛏ;ÅмCΒΜηT:(ᛘrφjζτаfΥнᛈwᚺᚷ]ÐᛁgßSáᚬл@ΛЯᛰОÏМ/Бøξpᚢαdу*шᛇeODВΠWBРxÄbω1ЮэАγvᚨᛗ[%KЛ^ΕеСλu9з}éþйΟᛮЧæc.ÍΑïЙЕΗθЪ$ΨÉ5ᛋμlóπЁЖр36ёX4_ЭqÞYᛜ~ᛊПυρИюᛃК!ᛖᚴκиhsЬΩΡжбiдÓäUοΧ<осΘ\'FᛦmᛟERᛒД2δЫ,ъψт`ö=ItzТчΔᛯHШЗ>ΚУΓΣσχфᚹᛞΖð8ЦØпГяцnФÁΞ|ᚲNíΙyщНкβ', ['!', '$', ',', '-', '/', '3', '8', '9', ':', ';', '?', 'A', 'G', 'I', 'J', 'K', 'M', 'S', 'V', 'W', 'X', '^', 'b', 'f', 't', 'v', 'y', '{', '}', 'Á', 'É', 'Ø', 'ð', 'þ', 'æ', 'Α', 'Γ', 'Ζ', 'Θ', 'Ι', 'Κ', 'Λ', 'Ο', 'Ψ', 'β', 'ε', 'θ', 'ι', 'κ', 'μ', 'ξ', 'σ', 'υ', 'χ', 'В', 'Г', 'Д', 'Ё', 'Л', 'О', 'Т', 'У', 'Ч', 'Ы', 'Э', 'а', 'ё', 'з', 'й', 'к', 'л', 'н', 'р', 'т', 'ц', 'ч', 'ш', 'ъ', 'ы', 'ᚨ', 'ᚲ', 'ᚴ', 'ᚼ', 'ᛇ', 'ᛈ', 'ᛊ', 'ᛏ', 'ᛚ', 'ᛞ', 'ᛯ']],
    # }
    ...
    #     "B": ["B", "ᛰᛯᛮᛦᛟᛞᛜᛚᛘᛗᛖᛒᛏᛋᛊᛉᛈᛇᛅᛃᛁᚾᚼᚺᚹᚷᚴᚲᚱᚬᚨᚦᚢᚠяюэьыъщшчцхфутсрпонмлкйизжёедгвбаЯЮЭЬЫЪЩШЧЦХФУТСРПОНМЛКЙИЗЖЁЕДГВБАωψχφυτσρποξνμλκιθηζεδγβαΩΨΧΦΥΤΣΡΠΟΞΝΜΛΚΙΘΗΖΕΔΓΒΑßåöøäæþóðïíéáÅÖØÄÆÞÓÐÏÍÉÁ~}|{zyxwvutsrqponmlkjihgfedcba`_^][ZYXWVUTSRQPONMLKJIHGFEDCBA@?>=<;:9876543210/.-,+*)('&%$#\"!"],
...
def choose_letters():
    a0 = "!\"#$%&'()*+,-./0123456789:;<=>?"
    # Not including "\" as this creates problems.
